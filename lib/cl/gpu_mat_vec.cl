...
struct XFORM const vvcxform( struct VEC const _x1, struct VEC const _x2,
...
inline struct XFORM const stubrev( struct VEC const a, struct VEC const b, struct VEC const c);
inline struct XFORM const stubc( struct VEC const cen, struct VEC const a, struct VEC const b, struct VEC const c);
inline struct XFORM const stubcrev( struct VEC const cen, struct VEC const a, struct VEC const b, struct VEC const c);
...
inline float dist2v( struct VEC u,  struct VEC v) {
  return mad(u.x-v.x,u.x-v.x,mad(u.y-v.y,u.y-v.y,(u.z-v.z)*(u.z-v.z)));
}
...
inline struct MAT projection_matrixf(float const x, float const y, float const z) {
  struct MAT P;
  float l2 = native_recip(length2f(x,y,z));
